...
def main():
	...
	numbers = []
	formatted_input = inputs_values.split()
	...
	for line in formatted_input:
		...
		for i in range(len(line)):
			...
			for j in range(len(line)):
				if line[i : j + 1] in num_dict:
					nums.append(int(num_dict[line[i : j + 1]]))
		numbers.append(nums)
	print(numbers)
	ans = 0
	for line in numbers:
...
def part1():
	...
	for string in formatted_input:
		number = ''
		digs = re.findall(r'\d', string)
		print(digs)

		if len(digs) == 1:
			number = str(int(digs[0]) * 11)
			numbers.append(number)
			continue
		if len(digs) == 0:
			pass
		else:
			number = digs[0] + digs[-1]
			numbers.append(number)
			continue


	int_nums = [int(i) for i in numbers]
	print(numbers)
	print(sum(int_nums))
